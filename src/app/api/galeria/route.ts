import { NextResponse } from "next/server";
  ...
  export async function POST(req: Request) {
    ...
    if (!file || !categoria) return NextResponse.json({ error: "Arquivo e categoria são obrigatórios" }, {
  status: 400 });
    if (!isValidImage(file)) return NextResponse.json({ error: "Arquivo inválido ou muito grande" }, {
  status: 400 });
